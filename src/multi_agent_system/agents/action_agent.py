...
class ActionAgent(BaseAgent):
    """Agent for action management without technical leaks."""
    ...
    def _build_action_system_prompt(self, prompts_dict):
        core_identity = prompts_dict.get('00_core_identity', 'You are a helpful action assistant.')
        action_schema = prompts_dict.get('01_action_schema', '')
        leak_prevention = """
        
CRITICAL: Help with actions naturally. Never include:
...
Respond like a helpful assistant managing actions and tasks.
        """
        return f"{core_identity}\n\n{action_schema}{leak_prevention}"
    ...
    async def process(self, user_input, context, routing_info=None):
        try:
            ...
            system_prompt = self.comprehensive_prompts.get('core_system', 'You are a helpful action assistant.')
            
            user_prompt = f"""
User wants to perform an action: {user_input}

Help them with their action or task. Be proactive and helpful.
...
"""
            ...
            if user_id:
                self._log_action(
                    ...
                    action_details={"type": "action_management"},
                    success_status=True
                )
            ...
            print(f"ERROR in ActionAgent: {e}")
            return {
                "message": "I'd be happy to help you with that action. What specifically would you like me to do?"
            }
