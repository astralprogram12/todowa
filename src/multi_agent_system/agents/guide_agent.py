...
class GuideAgent(BaseAgent):
    ...
    def _build_guide_system_prompt(self, prompts_dict):  # Customize this helper for each agent
        """Builds the system prompt for the Guide agent."""
    ...
    async def process(self, user_input, context, routing_info=None):
        """Process guidance and tutorial requests."""
        try:
...
Process this guidance request following all prompt guidelines.
"""
            ...
            if user_id:
                ...
                database.log_action(
                    ...
                    action_details={
                        "guidance_type": assumptions.get('guidance_type', 'general')
                    },
                    success_status=True
                )
            ...
            return {
                ...
                "actions": ["guidance_processed"]
            }
            
        except Exception as e:
            return {
                "message": "I encountered an error while processing the guidance request.",
                "actions": ["guide_error"],
                "error": str(e)
            }
