...
def webhook():
    ...
    try:
        ...
        # --- AGENT RUN (MODIFIED BLOCK) ---
        print("\n--- AGENT RUN ---")
        
        # STEP 1: Fetch all necessary context for the AI
        task_data = database.get_task_context_for_ai(supabase, user_id)
        list_data = database.get_list_context_for_ai(supabase, user_id)
        user_data = database.get_user_context_for_ai(supabase, user_id) # <-- NEW
        
        # STEP 2: Combine them into the single context object the agent expects
        full_context = {
            "tasks": task_data.get("tasks", []),
            "list_context": list_data,
            "user_context": user_data  # <-- NEW
        }
        
        # STEP 3: Call the agent with the full context
